...
// DONE_TODO: Provide a URL that can be used to download a certificate that can be used
// to verify JWT token signature.
// To get this URL you need to go to an Auth0 page -> Show Advanced Settings -> Endpoints -> JSON Web Key Set
const jwksUrl = 'https://vukhiemtinh.jp.auth0.com/.well-known/jwks.json'

export const handler = async (
  event: CustomAuthorizerEvent
): Promise<CustomAuthorizerResult> => {
  logger.info('user is authorizing', event.authorizationToken)
  try {
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  // DONE_TODO: Implement token verification
